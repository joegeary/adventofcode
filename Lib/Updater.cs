...
internal class Updater
{
    ...
    public async Task Update(int year, int day, string session)
    {
        session ??= GetSession();
        var baseAddress = new Uri("https://adventofcode.com/");

        var requester = new DefaultHttpRequester("github.com/joegear/adventofcode");
    ...
    {
        if (!Environment.GetEnvironmentVariables().Contains("AOC_SESSION"))
        {
            throw new AocCommuncationException("Specify AOC_SESSION environment variable", null);
        }
        return Environment.GetEnvironmentVariable("AOC_SESSION");
    }
    ...
    public async Task Upload(ISolver solver, string session)
    {
        session ??= GetSession();
        ...
        else
        {
            ...
            if (article.StartsWith("That's the right answer") || article.Contains("You've finished every puzzle"))
            {
                ...
                Console.ForegroundColor = color;
                Console.WriteLine();
                await Update(solver.Year(), solver.Day(), session);
            }
    ...
    string Dir(int year, int day) => ISolverExtensions.WorkingDir(year, day);
    ...
    {
        var file = Path.Combine(ISolverExtensions.WorkingDir(calendar.Year), "README.md");
        WriteFile(file, new YearReadmeGenerator().Generate(calendar));

        var svg = Path.Combine(ISolverExtensions.WorkingDir(calendar.Year), "calendar.svg");
        WriteFile(svg, calendar.ToSvg());
    }

    void UpdateSplashScreen(Calendar calendar)
    {
        var file = Path.Combine(ISolverExtensions.WorkingDir(calendar.Year), "SplashScreen.cs");
